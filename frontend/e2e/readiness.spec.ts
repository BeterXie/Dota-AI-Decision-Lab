...
test("keeps the public performance route focused on leaderboard data without duplicating readiness reports", async ({ page }) => {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (path === "/api/review/ai-quality/readiness") readinessRequests += 1;
