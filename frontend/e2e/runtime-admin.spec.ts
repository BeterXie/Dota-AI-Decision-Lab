...
async function mockRuntimeAdmin(page: Page): Promise<RuntimeHarness> {
  ...
  const providers = () => [
    ...
    provider("gemini", "default", "gemini-3.6-flash", true, false, configuredSecrets.has("ai.gemini.api_key"), null, 30),
  ...
  const secretItems = () => [
    secretStatus("auth.google.client_secret", "Google Client Secret", "authentication", configuredSecrets),
    secretStatus("ai.openai.api_key", "OpenAI API Key", "ai", configuredSecrets),
    secretStatus("ai.local_openai.api_key", "Local OpenAI-compatible API Key", "ai", configuredSecrets),
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (path.startsWith("/api/admin/runtime/policy/") && request.method() === "PATCH") {
      ...
      await json(route, setting(key, body.value, key.startsWith("feature.") ? "feature" : "ai_decision"));
      return;
    }
...
test("runtime overview uses the full admin shell without loading match directory data", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /AI 决策设置/ })).toHaveAttribute("href", "/admin/runtime/ai-decisions");
  await expect(page.getByRole("link", { name: /功能开关/ })).toHaveAttribute("href", "/admin/runtime/features");
  await expect(page.getByRole("link", { name: /外部服务 \/ Secrets/ })).toHaveAttribute("href", "/admin/runtime/secrets");
...
test("AI decision settings hot-update the PREPARE policy and keep worker concurrency honest", async ({ page }) => {
  const harness = await mockRuntimeAdmin(page);
  ...
  await lag.fill("90");
  await lag.locator("xpath=ancestor::section").getByRole("button", { name: "保存" }).click();

  await expect.poll(() => harness.patches.some((entry) => entry.path.endsWith("/ai.max_live_data_lag_seconds"))).toBe(true);
  expect(harness.patches.find((entry) => entry.path.endsWith("/ai.max_live_data_lag_seconds"))?.body).toEqual({ value: 90 });
  await expect(page.getByText(/运行时策略已更新；正在执行中的请求保持冻结配置/)).toBeVisible();
});

test("feature flags expose only real hot switches and PATCH performance gate", async ({ page }) => {
  const harness = await mockRuntimeAdmin(page);
  ...
  await expect.poll(() => harness.patches.some((entry) => entry.path.endsWith("/feature.performance.enabled"))).toBe(true);
  expect(harness.patches.find((entry) => entry.path.endsWith("/feature.performance.enabled"))?.body).toEqual({ value: false });
});
...
test("secrets page replaces a missing key without ever rendering plaintext", async ({ page }) => {
  ...
  await expect.poll(() => harness.patches.some((entry) => entry.path.endsWith("/ai.gemini.api_key"))).toBe(true);
  expect(harness.patches.find((entry) => entry.path.endsWith("/ai.gemini.api_key"))?.body).toEqual({ value: plaintext });
  await expect(page.getByText(plaintext, { exact: true })).toHaveCount(0);
  await expect(page.getByRole("row").filter({ hasText: "Gemini API Key" }).getByText("已配置", { exact: true })).toBeVisible();
});
...
function secretStatus(key: string, label: string, category: string, configured: Set<string>) {
  return {
    ...
    storage: configured.has(key) ? "DATABASE_ENCRYPTED" : "NOT_CONFIGURED",
    runtime_hot: true
  };
}
