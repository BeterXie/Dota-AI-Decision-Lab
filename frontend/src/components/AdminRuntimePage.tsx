...
function FeatureFlagsPage({ policy, locale, busy, onPolicy }: { policy: RuntimePolicyPayload; locale: string; busy: boolean; onPolicy: (key: string, value: unknown) => Promise<void> }) {
  return <div className="admin-page-stack">
    ...
    <section className="admin-feature-grid">
      ...
      <FeatureCard icon="$" title={zh(locale, "Billing 新购买", "New billing checkout")} detail={zh(locale, "关闭新 checkout，保留账户查询与 Paddle webhook 对账", "Disables new checkout while account reads and Paddle webhooks remain active")} checked={policyBool(policy, "feature.billing_checkout.enabled")} disabled={busy} onChange={(value) => void onPolicy("feature.billing_checkout.enabled", value)} />
    </section>
