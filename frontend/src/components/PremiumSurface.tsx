...
function premiumIntroCopy(surface: PremiumSurfaceKey, zh: boolean) {
  if (surface === "performance") {
    return {
      ...
      description: zh
        ? "所有用户都可以查看模型在统一预测积分规则和初始积分下的长期表现。积分不可充值、提现、转让或兑换。"
        : "Everyone can compare long-term model performance under the same prediction-points rules and initial points. Points cannot be purchased, withdrawn, transferred, or redeemed.",
      notes: zh ? ["统一积分规则", "积分排行榜", "不可兑换"] : ["Same points rules", "Points leaderboard", "No redemption"]
    };
  }
  if (surface === "review") {
    return {
      ...
      description: zh
        ? "所有用户都可以把比赛结果、关键市场变化和 AI 预测放回同一条时间线上，查看模型为什么这样预测，以及最后是否经得住赛后验证。"
        : "Everyone can put the result, important market changes, and AI predictions back on one timeline and inspect how each model held up after the match.",
  ...
  if (surface === "billing") {
    return {
      ...
      description: zh
        ? "小组赛与确认赛果后的基础 AI 预测公开。系列赛或赛事 Pass 解锁进行中的完整 AI 预测与实时通知。"
        : "Group-stage and confirmed post-match core AI predictions are public. A Series or Event Pass unlocks full live predictions and realtime alerts.",
...
function accessCopy(_kind: "notifications", locale: string) {
  ...
  return {
    ...
    description: zh
      ? "解锁赛事 Pass 或系列赛 Pass 后，可以绑定邮箱、QQ 和微信；系统只会发送你仍然有权限查看的比赛。"
      : "After unlocking an Event Pass or Series Pass, you can bind email, QQ or WeChat. Alerts are sent only for matches your account can still access.",
